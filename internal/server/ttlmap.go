...
func (m *TTLMap) Put(k string, v *UserSession) error {
	m.l.Lock()
	_, ok := m.m[k]
	var it *item
	if !ok {
		...
		return errors.New("failed to put key: " + k + ", value: " + v.Username)
	}
...
func (m *TTLMap) Get(k string) (v *UserSession, err error) {
	m.l.Lock()
	if it, ok := m.m[k]; ok {
		v = it.value
		it.lastAccess = time.Now().Unix()
